...
export default function Home({ params: { slug } }: ParamsProps) {
  ...
  useEffect(() => {
    ...
    // Standardize slug parsing for both direct and encoded formats
    let parsedSlug = decodeURIComponent(slug);
    if (parsedSlug.startsWith("to:")) {
      setName(parsedSlug.slice(3).trim());
    }

    const contentTimer = setTimeout(() => {
      // Only transition if not in preview mode
      if (!isPreview) {
        setShowContent(true);
      }
    }, 7000);
